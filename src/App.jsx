...
const EMPTY = { nome: "", tipo: "Frontlight", reg: "", end: "", preco: "", dim: "", ilum: "", obs: "", img_url: null };
...
function Card({ p, selMode, selected, onToggle, onEdit, onDelete }) {
  return (
    <div onClick={selMode ? onToggle : undefined}
      style={{ background: "white", borderRadius: 12, overflow: "hidden", border: selected ? "2px solid #1B6CA8" : "1px solid #e8e8e8", cursor: selMode ? "pointer" : "default", boxShadow: selected ? "0 0 0 3px rgba(27,108,168,0.15)" : "0 1px 4px rgba(0,0,0,0.06)" }}>
      <div style={{ width: "100%", height: 130, background: "#f5f5f5", position: "relative", overflow: "hidden" }}>
        {p.img_url
          ? <img src={p.img_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
          : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "#ccc", fontSize: 28 }}>📷</div>}
        {selMode && (
          <div style={{ position: "absolute", top: 8, left: 8, width: 22, height: 22, borderRadius: "50%", background: selected ? "#1B6CA8" : "rgba(255,255,255,0.92)", border: selected ? "2px solid #1B6CA8" : "2px solid #ccc", display: "flex", alignItems: "center", justifyContent: "center", color: selected ? "#fff" : "transparent", fontSize: 12 }}>✓</div>
        )}
      </div>
...
function Modal({ inicial, onSave, onClose, saving }) {
  ...
  const handleImg = async (e) => {
    ...
    setUploading(true);
    try { const url = await uploadFoto(file); set("img_url", url); }
    catch(err) { alert("Erro ao enviar foto. Tente novamente."); console.error(err); }
    finally { setUploading(false); }
  };

  const inp = (k, ph) => <input value={form[k]||""} onChange={e=>set(k,e.target.value)} placeholder={ph} style={{ width:"100%",height:36,padding:"0 10px",fontSize:13,border:"1px solid #e0e0e0",borderRadius:8,fontFamily:"inherit",background:"#fafafa",boxSizing:"border-box" }} />;
  const lbl = t => <label style={{ display:"block",fontSize:11,color:"#888",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.05em" }}>{t}</label>;
    ...
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div style={{ background:"white",borderRadius:16,padding:24,width:500,maxWidth:"100%",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ marginBottom:14 }}>
          {lbl("Foto do ponto")}
          <div onClick={()=>!uploading&&fileRef.current.click()} style={{ border:"1.5px dashed #ddd",borderRadius:10,padding:16,textAlign:"center",cursor:uploading?"default":"pointer",fontSize:13,color:"#999" }}>
            {uploading?<><Spinner/>&nbsp;Enviando foto...</>:"📷 Clique para adicionar foto"}
            <input ref={fileRef} type="file" accept="image/*" style={{ display:"none" }} onChange={handleImg} />
          </div>
          {form.img_url && <img src={form.img_url} alt="" style={{ width:"100%",height:150,objectFit:"cover",borderRadius:8,marginTop:8 }} />}
        </div>
        <div style={{ marginBottom:12 }}>{lbl("Nome do ponto *")}{inp("nome","Ex: Frontlight Zona Sul #01")}</div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12 }}>
          <div>{lbl("Tipo de mídia *")}
            <select value={form.tipo} onChange={e=>set("tipo",e.target.value)} style={{ width:"100%",height:36,padding:"0 10px",fontSize:13,border:"1px solid #e0e0e0",borderRadius:8,background:"#fafafa",fontFamily:"inherit" }}>
              {TIPOS.map(t=><option key={t}>{t}</option>)}
            </select>
          </div>
          <div>{lbl("Região *")}{inp("reg","Ex: Zona Sul, Centro...")}</div>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12 }}>
          <div>{lbl("Endereço")}{inp("end","Rua, número...")}</div>
          <div>{lbl("Preço mensal (R$)")}{inp("preco","Ex: 3.500,00")}</div>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12 }}>
          <div>{lbl("Dimensões")}{inp("dim","Ex: 9m x 3m")}</div>
          <div>{lbl("Iluminação")}
            <select value={form.ilum||""} onChange={e=>set("ilum",e.target.value)} style={{ width:"100%",height:36,padding:"0 10px",fontSize:13,border:"1px solid #e0e0e0",borderRadius:8,background:"#fafafa",fontFamily:"inherit" }}>
              <option value="">—</option><option>Sim</option><option>Não</option>
            </select>
          </div>
        </div>
        <div style={{ marginBottom:20 }}>{lbl("Observações")}
          <textarea value={form.obs||""} onChange={e=>set("obs",e.target.value)} placeholder="Visibilidade, tráfego, disponibilidade..." style={{ width:"100%",height:70,padding:"8px 10px",fontSize:13,border:"1px solid #e0e0e0",borderRadius:8,resize:"vertical",fontFamily:"inherit",background:"#fafafa",boxSizing:"border-box" }} />
        </div>
        <div style={{ display:"flex",justifyContent:"flex-end",gap:8 }}>
          ...
          <button onClick={()=>onSave(form)} disabled={saving||uploading} style={{ padding:"8px 20px",fontSize:13,borderRadius:8,border:"none",background:(saving||uploading)?"#e0e0e0":"#C1272D",color:(saving||uploading)?"#999":"white",cursor:(saving||uploading)?"default":"pointer",fontWeight:600,fontFamily:"inherit",display:"flex",alignItems:"center",gap:8 }}>
...
function buildPDF(lista, cli, emp) {
  ...
  const rows = lista.map(p => {
    ...
    const imgTag = p.img_url
      ? `<img src="${p.img_url}" style="width:148px;height:104px;object-fit:cover;border-radius:4px;display:block;">`
    ...
    return `<div style="display:flex;gap:16px;padding:18px 0;border-bottom:0.5px solid #e8e8e8;">
  <div style="flex-shrink:0;">${imgTag}</div>
  <div style="flex:1;">
    <div style="display:inline-block;font-size:9px;padding:2px 8px;border-radius:20px;font-weight:600;text-transform:uppercase;margin-bottom:5px;background:${c.bg};color:${c.tx};">${p.tipo}</div>
...
<div style="background:linear-gradient(135deg,#C1272D 0%,#8B1B1F 100%);padding:26px 30px 20px;display:flex;justify-content:space-between;align-items:flex-end;">
  <div><div style="font-size:24px;font-weight:700;color:#fff;line-height:1;">Hoc&gt;&gt;✕&lt;</div><div style="font-size:9px;letter-spacing:.2em;color:rgba(255,255,255,.7);margin-top:3px;">SMARTMIDIA</div></div>
  <div style="text-align:right;color:rgba(255,255,255,.85);font-size:11px;line-height:1.7;">
    <div style="color:#fff;font-size:13px;font-weight:600;">${cli}</div>${emp?`<div>${emp}</div>`:""}
    <div>Data: ${data}</div>
  </div>
</div>
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try {
        const {data,error}=await supabase.from("pontos").select("id,nome,tipo,reg,end,preco,dim,ilum,obs,img_url").order("id");
        if(error) throw error;
        setPontos(data||[]);
        setDbOk(true);
      } catch(e) {
  ...
  const salvar=async(form)=>{
    ...
    setSaving(true);
    try {
      if(form.id&&pontos.find(p=>p.id===form.id)){
        const {error}=await supabase.from("pontos").update({nome:form.nome,tipo:form.tipo,reg:form.reg,end:form.end,preco:form.preco,dim:form.dim,ilum:form.ilum,obs:form.obs,img_url:form.img_url}).eq("id",form.id);
        if(!error) setPontos(prev=>prev.map(p=>p.id===form.id?{...p,...form}:p));
      } else {
        const {data,error}=await supabase.from("pontos").insert([{nome:form.nome,tipo:form.tipo,reg:form.reg,end:form.end,preco:form.preco,dim:form.dim,ilum:form.ilum,obs:form.obs,img_url:form.img_url}]).select().single();
  ...
  const invFilt=pontos.filter(p=>(!fTipo||p.tipo===fTipo)&&(!fReg||p.reg===fReg)&&(!search||p.nome.toLowerCase().includes(search.toLowerCase())||(p.reg||"").toLowerCase().includes(search.toLowerCase())));
  const propFilt=pontos.filter(p=>(!fTipoProp||p.tipo===fTipoProp)&&(!fRegProp||p.reg===fRegProp));

  const gerarPDF=()=>{if(!sel.size){alert("Selecione ao menos um ponto.");return;}setPdfHTML(buildPDF(pontos.filter(p=>sel.has(p.id)),cliNome||"Cliente",cliEmp));};

  const baixarPDF=()=>{
    if(!pdfHTML)return;
    const cli=(cliNome||"Cliente").replace(/\s/g,"_");
    const data=new Date().toLocaleDateString("pt-BR").replace(/\//g,"-");
    const html=`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Proposta Hocxx</title><style>body{margin:0;padding:0;font-family:'Helvetica Neue',sans-serif;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body><div style="max-width:720px;margin:0 auto;">${pdfHTML}</div><script>window.onload=function(){window.print();}<\/script></body></html>`;
    const blob=new Blob([html],{type:"text/html"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url;a.download=`Proposta_Hocxx_${cli}_${data}.html`;a.click();URL.revokeObjectURL(url);
  };
  ...
  if(loading)return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:400,gap:16}}><Spinner/><span style={{fontSize:14,color:"#888"}}>Carregando inventário...</span></div>);
  if(!dbOk)return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:400,gap:16,padding:32,textAlign:"center"}}><div style={{fontSize:40}}>⚠️</div><h2 style={{fontSize:18,color:"#C1272D"}}>Banco de dados não configurado</h2><p style={{fontSize:13,color:"#666",lineHeight:1.6}}>Abra o arquivo <code>src/App.jsx</code> e substitua<br/><code>SUPABASE_URL</code> e <code>SUPABASE_KEY</code><br/>com as suas chaves do Supabase.</p></div>);
    ...
    <>
      ...
      <div style={{minHeight:"100vh",fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f9f9f9"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",height:52,background:"white",borderBottom:"1px solid #eee",position:"sticky",top:0,zIndex:50}}>
          <div><Logo size={18}/><div style={{fontSize:9,letterSpacing:"0.18em",color:"#aaa",textTransform:"uppercase",marginTop:1}}>Smartmidia</div></div>
          <div style={{display:"flex",gap:4}}>
            {[["inventario","🗂 Inventário"],["proposta","📄 Nova Proposta"]].map(([id,label])=>(
              <button key={id} onClick={()=>{setScreen(id);if(id==="proposta"){setSel(new Set());setPdfHTML(null);}}}
                style={{padding:"6px 14px",borderRadius:8,fontSize:13,cursor:"pointer",border:"1px solid",fontFamily:"inherit",borderColor:screen===id?"#e0e0e0":"transparent",background:screen===id?"#f5f5f5":"transparent",fontWeight:screen===id?600:400,color:screen===id?"#111":"#888"}}>
                {label}
              </button>
            ))}
          </div>
        ...
        {screen==="inventario"&&(
          <div style={{padding:"18px 20px"}}>
            <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
              ...
              <select value={fTipo} onChange={e=>setFTipo(e.target.value)} style={sel$}><option value="">Todos os tipos</option>{TIPOS.map(t=><option key={t}>{t}</option>)}</select>
              <select value={fReg} onChange={e=>setFReg(e.target.value)} style={sel$}><option value="">Todas as regiões</option>{regioes.map(r=><option key={r}>{r}</option>)}</select>
              <button onClick={()=>setModal({form:{...EMPTY}})} style={{padding:"0 16px",height:34,background:"#C1272D",color:"white",border:"none",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>+ Novo Ponto</button>
            </div>
            {invFilt.length===0
              ?<div style={{textAlign:"center",padding:"60px 20px",color:"#bbb"}}><div style={{fontSize:40,marginBottom:12}}>🗺️</div><div style={{fontSize:14}}>Nenhum ponto encontrado.<br/>Clique em "+ Novo Ponto" para começar.</div></div>
              :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:12}}>
                {invFilt.map(p=><Card key={p.id} p={p} selMode={false} onEdit={()=>setModal({form:{...p}})} onDelete={()=>excluir(p.id)}/>)}
              </div>}
          </div>
        )}
        ...
        {screen==="proposta"&&(
          <div style={{padding:"18px 20px 100px"}}>
            <div style={{background:"#EBF4FB",border:"1px solid #B5D4F4",borderRadius:10,padding:"9px 13px",fontSize:12,color:"#1B5080",display:"flex",alignItems:"center",gap:8,marginBottom:16}}>ℹ️ Selecione os pontos, preencha os dados do cliente e gere o PDF.</div>
            ...
            <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              <select value={fTipoProp} onChange={e=>setFTipoProp(e.target.value)} style={sel$}><option value="">Todos os tipos</option>{TIPOS.map(t=><option key={t}>{t}</option>)}</select>
              <select value={fRegProp} onChange={e=>setFRegProp(e.target.value)} style={sel$}><option value="">Todas as regiões</option>{regioes.map(r=><option key={r}>{r}</option>)}</select>
            ...
            {propFilt.length===0
              ...
              :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:12}}>
                {propFilt.map(p=><Card key={p.id} p={p} selMode={true} selected={sel.has(p.id)} onToggle={()=>toggleSel(p.id)}/>)}
              </div>}
            {pdfHTML&&(
              <div style={{marginTop:24}}>
                ...
                <div style={{background:"#f0f0ee",borderRadius:10,padding:14}}>
                  <div style={{background:"white",borderRadius:4,overflow:"hidden",maxWidth:580,margin:"0 auto"}} dangerouslySetInnerHTML={{__html:pdfHTML}}/>
                </div>
              </div>
            )}
            <div style={{position:"fixed",bottom:0,left:0,right:0,background:"white",borderTop:"1px solid #eee",padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",zIndex:40}}>
              ...
              <button onClick={gerarPDF} style={{padding:"8px 20px",fontSize:13,borderRadius:8,border:"none",background:"#1B6CA8",color:"white",cursor:"pointer",fontWeight:600,fontFamily:"inherit"}}>📄 Gerar Proposta PDF</button>
            </div>
          </div>
        )}
